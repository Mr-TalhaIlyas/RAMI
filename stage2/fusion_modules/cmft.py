...
class FusionNet(nn.Module):
    ...
    def forward(self, img_feat1, img_feat2, img_feat3):
        img_feat1 = img_feat1.squeeze(1).unsqueeze(-1).unsqueeze(-1)
        img_feat2 = img_feat2.squeeze(1).unsqueeze(-1).unsqueeze(-1)
        img_feat3 = img_feat3.squeeze(1).unsqueeze(-1).unsqueeze(-1)
        # 对channel做attention
...
class CMFT(nn.Module):
    def __init__(self, backbone_dim=256, c_dim=256, num_c=3, chkpt=None):
        super(CMFT, self).__init__()

        self.fusion = FusionNet(backbone_dim, c_dim, num_c)
        if chkpt:
            self.fusion.load_state_dict(torch.load(chkpt))
            print('Loaded FusionNet Module')
    
    def forward(self, img_feat1, img_feat2, img_feat3):
        return self.fusion(img_feat1, img_feat2, img_feat3)
    
#%%    
# model = FusionNet(backbone_dim=256, c_dim=256, num_c=3)

# model.load_state_dict(torch.load('/home/talha/Data/mme/gp_test/fusion_modules/chkpt/cmft1.pth'))
...
# os.chdir('/home/talha/Data/mme/gp_test/')
# import torch
...
# chkpt = "/home/talha/Data/mme/chkpts/cmft_fold5.pth"
...
#            '/home/talha/Data/mme/gp_test/fusion_modules/chkpt/cmft5.pth')
# %%
