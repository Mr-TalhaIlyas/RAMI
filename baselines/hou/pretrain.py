...
from tools import pretrain_tcnp, pretrain_tcnf, tcn_inference
import sklearn.metrics as skm
#%%
...
# tcnp_model.load_state_dict(torch.load('tcnp_pretrained.pth'))
# tcnf_model.load_state_dict(torch.load('tcnf_pretrained.pth'))

# Optimizers
optimizer_p = torch.optim.SGD(tcnp_model.parameters(), lr=config['learning_rate'])
optimizer_f = torch.optim.SGD(tcnf_model.parameters(), lr=config['learning_rate'])
...
for epoch in range(config['num_epochs']):
    ...
    for _ in range(config['num_repeats_per_epoch']):
        ce_lossp = pretrain_tcnp(tcnp_model, train_loader, optimizer_p)
        p_scheduler.step()
        
        ce_lossf = pretrain_tcnf(tcnf_model, train_loader, optimizer_f)
        f_scheduler.step()
        # append losses
        total_lossesp.append(ce_lossp)
        total_lossesf.append(ce_lossf)
    
    total_lossesp = np.nanmean(total_lossesp)
    total_lossesf = np.nanmean(total_lossesf)
    print(f'Epoch: {epoch+1}/{config["num_epochs"]}::')
    print(f'=> Average body loss: {total_lossesp:.4f}')
    print(f'=> Average face loss: {total_lossesf:.4f}')
          
    # evaluate 
    val_acc, val_f1, val_rec, val_prec = [], [], [], []
    for _ in range(config['num_repeats_per_epoch']):
        preds, labels = tcn_inference(tcnp_model, tcnf_model, test_loader)
    ...
    if curr_f1 > best_f1:
        ...
        best_f1 = curr_f1
        torch.save(tcnp_model.state_dict(), f"{config['checkpoint_path']}tcnp_pretrained_{config['num_fold']}.pth")
        torch.save(tcnf_model.state_dict(), f"{config['checkpoint_path']}tcnf_pretrained_{config['num_fold']}.pth")
