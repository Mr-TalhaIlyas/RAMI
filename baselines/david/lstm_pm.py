...
class LSTM_PM(nn.Module):
    ...
    def __init__(self, outclass=21, T=7):
        ...
        # Fully connected layer to extract 4096-dimensional features
        self.global_avg_pool = nn.AdaptiveAvgPool2d((1, 1))
        self.fc_features = nn.Linear(512 * 1 * 1, 4096)
    ...
    def convnet1(self, image):
        ...
        x_g = self.global_avg_pool(x)  # Output: [batch_size, 512, 1, 1]
        x_flat = x_g.view(x_g.size(0), -1)                               # batch_size x (512*45*45)
        features = self.fc_features(x_flat)                          # batch_size x 4096

        initial_heatmap = self.conv7_convnet1(x)                     # outclass x 45 x 45
        return initial_heatmap, features
    ...
    def stage1(self, image, cmap):
        ...
        hide_t:                      48 * 45 * 45
        '''
        initial_heatmap, _ = self.convnet1(image)
        features = self.convnet2(image)
        centermap = self.pool_center_lower(cmap)
    ...
    def forward(self, images, center_map):  
        '''
        :param images: Tensor (batch_size x (T*3) x H x W)
        :param center_map: Tensor (batch_size x 1 x H x W)
        :return:
        features_list: List of feature vectors (batch_size x T x 4096)
        heat_maps: List of heatmaps (batch_size x (T + 1) x outclass x 45 x 45)
        '''  
        ...
        image = images[:, 0:3, :, :]
        # print(image.shape)
        initial_heatmap, features = self.convnet1(image)
        initial_heatmap, heatmap, cell, hide = self.stage1(image, center_map)
        ...
        for i in range(1, self.T):
            ...
            _, features = self.convnet1(image)
            features_list.append(features)
            heat_maps.append(heatmap)

        # Stack features and heatmaps
        features_tensor = torch.stack(features_list, dim=1)  # batch_size x T x 4096
        heatmaps_tensor = torch.stack(heat_maps, dim=1)      # batch_size x (T + 1) x outclass x 45 x 45

        return features_tensor, heatmaps_tensor
#%%

# net = LSTM_PM(T=4)
# a = torch.randn(2, 12, 368, 368)  # batch size = 2
# c = torch.randn(2, 1, 368, 368) # <- center map guassian heatmap
# feats, maps = net(a, c)
# for f, m in zip(maps, feats):
#    print(m.shape, f.shape)
# feats <- batch_size x T x 4096 optical flow features
#%%